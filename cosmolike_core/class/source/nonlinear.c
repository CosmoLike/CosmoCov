...
int nonlinear_init(
  ...
  /** --> Then go through common preliminary steps to the HALOFIT and HMcode methods */
  else if ((pnl->method == nl_halofit) || ((pnl->method == nl_HMcode)) || ((pnl->method == nl_HMcode_2020))) {
	...
	if ((pnl->nonlinear_verbose > 0) && (pnl->method == nl_HMcode_2020))
      printf("Computing non-linear matter power spectrum with HMcode2020 \n");
    ...
    if (pnl->method == nl_HMcode){
      ...
      class_call(nonlinear_hmcode_workspace_init(ppm, ppr,pba,pnl,pnw),
                 pnl->error_message,
                 pnl->error_message);
    ...
    for (index_tau = pnl->tau_size-1; index_tau>=0; index_tau--) {
      ...
      for (index_pk=0; index_pk<pnl->pk_size; index_pk++) {
        ...
        if (nl_corr_not_computable_at_this_k == _FALSE_) {
          ...
          /* get P_NL(k) at this time with HMcode */
          ...
          else if (pnl->method == nl_HMcode_2020) {
            /* (preliminary step: fill table of sigma's, only for _cb if there is both _cb and _m) */
            ...
            class_call(nonlinear_hmcode2020(ppr,
                                        pba,
                                        ppt,
                                        ppm,
                                        pnl,
...
int nonlinear_hmcode_workspace_init(
  ...
  class_alloc(pnw->intgrowtable, ppr->n_hmcode_mead_growth_tables*sizeof(double),pnl->error_message);
  class_alloc(pnw->ztable,ng*sizeof(double),pnl->error_message);
  class_alloc(pnw->scaletable,ng*sizeof(double),pnl->error_message);
  class_alloc(pnw->meadscaletable,ppr->n_hmcode_mead_growth_tables*sizeof(double),pnl->error_message);
  class_alloc(pnw->meadgrowtable,ppr->n_hmcode_mead_growth_tables*sizeof(double),pnl->error_message);
  class_alloc(pnw->dark_energy_correction_hmcode2020_table,ng*sizeof(double),pnl->error_message);
  ...
  class_call(nonlinear_hmcode_fill_meadgrowtab(ppr,pba,pnl,pnw),
             pnl->error_message,
             pnl->error_message);

  /** --fill table with G(a)=int_0^a g(a')/a' da' */

  class_call(nonlinear_hmcode_fill_intgrowtab(ppr,pba,pnl,pnw),
             pnl->error_message,
             pnl->error_message);
  class_call(fill_Pk_wiggle_table(ppm, ppr,pba,pnl,pnw), pnl->error_message, pnl->error_message);
...
int nonlinear_hmcode_workspace_free(
  ...
  free(pnw->growtable);
  free(pnw->meadgrowtable);
  free(pnw->meadscaletable);
  free(pnw->intgrowtable);
  free(pnw->ztable);
  free(pnw->scaletable);
  free(pnw->tautable);
  ...
  free(pnw->dark_energy_correction_hmcode2020_table);
  free(pnw->Pk_wiggle_tab);
...
int nonlinear_hmcode_dark_energy_correction(
      ...
      int last_index, index_scalefactor;
      double * pvecback;
      double tau_growth;
      double g_lcdm,g_wcdm;
      double g_lcdm_z, g_wcdm_z;
      ...
      if (pba->has_fld==_TRUE_){
        ...
        pnw->dark_energy_correction = pow(g_wcdm/g_lcdm, 1.5);
          for (index_scalefactor=0;index_scalefactor< ppr->n_hmcode_tables;index_scalefactor++){
              class_call(nonlinear_hmcode_growint(ppr,pba,pnl,pnw->scaletable[index_scalefactor],-1.,0.,&g_lcdm_z),
                   pnl->error_message, pnl->error_message);

              class_call(nonlinear_hmcode_growint(ppr,pba,pnl,pnw->scaletable[index_scalefactor],w0,dw_over_da_fld*(-1.),&g_wcdm_z),
                   pnl->error_message,
                   pnl->error_message);

              pnw->dark_energy_correction_hmcode2020_table[index_scalefactor] = g_wcdm/g_lcdm*g_lcdm_z/g_wcdm_z; 
          }
      }
      else {
        pnw->dark_energy_correction = 1.;
        for (index_scalefactor=0;index_scalefactor< ppr->n_hmcode_tables;index_scalefactor++){
            pnw->dark_energy_correction_hmcode2020_table[index_scalefactor] = 1.;
        }
      }
...
int nonlinear_hmcode_fill_growtab(
  ...
  for (index_scalefactor=0;index_scalefactor<ng;index_scalefactor++){
    ...
    pnw->scaletable[index_scalefactor] = scalefactor;
...
/**
 * Function that fills pnw->tautable and pnw->meadgrowtable with (tau, D(tau))
...
int meadgrowth_func(double a, const double y[], double f[], void *params) {
    ParamsMeadGrowth *p = (ParamsMeadGrowth *)params;
    f[0] = y[1];
...
int nonlinear_hmcode_fill_meadgrowtab(
  ...
  double z, ainit, amax, scalefactor, tau_growth;
  double a_init_mead=1E-4;
  int index_scalefactor, last_index, ng;
  double f;
  double y[2];
  int status;

  ng = ppr->n_hmcode_mead_growth_tables;
  ainit = ppr->ainit_for_growtab;
  amax = ppr->amax_for_growtab;

    ParamsMeadGrowth params = {pba->Omega0_m, pba->Omega0_de, pba->w0_fld, pba->wa_fld, pba->Omega0_k}; //   omega0m;
//    printf("%le %le %le %le %le\n\n\n", pba->Omega0_m, pba->Omega0_de, pba->w0_fld, pba->wa_fld, pba->Omega0_k);
    gsl_odeiv2_system sys = {meadgrowth_func, NULL, 2, &params};
    gsl_odeiv2_driver *driver = gsl_odeiv2_driver_alloc_y_new(&sys, gsl_odeiv2_step_rk8pd, 1e-6, 1e-6, 0.0);
    f = 1.-_Omega_m(a_init_mead, pba->Omega0_m, pba->Omega0_de,pba->Omega0_k, pba->w0_fld, pba->wa_fld);
    y[0] = pow(a_init_mead,(1.-3.*f/5.));
    y[1] = (1.-3.*f/5.)*pow(a_init_mead,(-3.*f/5.));


    for (index_scalefactor=0;index_scalefactor<ng;index_scalefactor++){
        scalefactor = ainit+(amax-ainit)*(index_scalefactor)/(ng-1);
        status = gsl_odeiv2_driver_apply(driver, &a_init_mead, scalefactor, y);
        if (status != GSL_SUCCESS) {
            printf("Error, return value=%d\n", status);
        }
        pnw->meadscaletable[index_scalefactor] = a_init_mead;
        pnw->meadgrowtable[index_scalefactor] = y[0];
    }
  return _SUCCESS_;
}
...
/**
 * Function that fill pnw->intgrowtable with (G(a)=int_0^a g(a')/a' da')
 * linearly spaced in scalefactor a.
 * Called by nonlinear_init at before the loop over tau
 *
 * @param ppr Input: pointer to precision structure
...
double integrand(double a, void *params){
    if (a==0) return 1;
    IntParam *p = ( IntParam *)params;
    double growth;

    class_call(array_interpolate_cubic_equal(p->pnw->meadscaletable[0],
                                             p->pnw->meadscaletable[1]-p->pnw->meadscaletable[0],
                                                           p->pnw->meadgrowtable,
                                                           p->ppr->n_hmcode_mead_growth_tables,
                                                           a,
                                                           &growth,
                                                           p->pnl->error_message),
                    p->pnl->error_message, p->pnl->error_message); 
    return growth/a;
}
...
int nonlinear_hmcode_fill_intgrowtab(
      ...
      for (index_scalefactor=0;index_scalefactor<ng;index_scalefactor++){
        gsl_integration_qags(&F, ainit, pnw->meadscaletable[index_scalefactor], 0, 1e-6, 1000, workspace, &result, &error);
        class_call(array_interpolate_cubic_equal(pnw->meadscaletable[0],
                                             pnw->meadscaletable[1]-pnw->meadscaletable[0],
                                                          pnw->meadgrowtable,
                                                           ppr->n_hmcode_mead_growth_tables,
                                                           ainit,
                                                           &growth,
                                                           pnl->error_message),
                   pnl->error_message, pnl->error_message); 
        ...
        result += growth; 
        pnw->intgrowtable[index_scalefactor] = result; 
      }
...
// Functions of k
    double Gamma = (wm/h) * (alpha + (1. - alpha) / (1. + pow(0.43 * k * s , 4))); // Equation (30)
    double q = (k/h) * pow(T_CMB/2.7, 2) / Gamma;   // Equation (28)
...
/**
 *  fill HMcode wiggle powerspectra
 *
 * @param ppm         Input: pointer to primordial structure
 * @param ppr         Input: pointer to precision structure
 * @param pba         Input: pointer to background structure
...
int fill_Pk_wiggle_table(
    ...
    double logkmin_wiggle = ppr->logkmin_wiggle+log(pba->h);
    double logkmax_wiggle = ppr->logkmax_wiggle+log(pba->h);
    static double sigma = 0.25;
    double dlnk = (logkmax_wiggle-logkmin_wiggle)/(ppr->nk_wiggle-1);
    double *Pk_nowiggle;
    double *Pk_ratio;
    double *pklin;
    int kin;
    double lnki;
    class_alloc(Pk_nowiggle,ppr->nk_wiggle*sizeof(double),pnl->error_message);
    class_alloc(pklin,ppr->nk_wiggle*sizeof(double),pnl->error_message);
    class_alloc(Pk_ratio,ppr->nk_wiggle*sizeof(double),pnl->error_message);
    class_alloc(pnw->Pk_wiggle_tab, ppr->nk_wiggle*sizeof(double),pnl->error_message);
    class_alloc(pnw->lnk_wiggle_tab,ppr->nk_wiggle*sizeof(double),pnl->error_message);
    lnki = logkmin_wiggle;
    double norm = 1;
            class_call(nonlinear_pk_at_k_and_z(pba,
                                       ppm,
               ...
               pnl->error_message,
               pnl->error_message);
    norm /= (pow(ppr->knorm_nowiggle*pba->h, ppm->n_s) * pow(Tk_EH_nowiggle(ppr->knorm_nowiggle*pba->h, pba->h, pba->Omega0_m*pba->h*pba->h, pba->Omega0_b*pba->h*pba->h, pba->T_cmb), 2));

    for (kin=0; kin<ppr->nk_wiggle; kin++, lnki += dlnk) {
            Pk_nowiggle[kin]= pow(exp(lnki), ppm->n_s) * pow(Tk_EH_nowiggle(exp(lnki), pba->h, pba->Omega0_m*pba->h*pba->h, pba->Omega0_b*pba->h*pba->h, pba->T_cmb), 2)*norm;
            class_call(nonlinear_pk_at_k_and_z(pba,
                                       ppm,
                                       pnl,
                                       pk_linear,
               ...
               pnl->error_message,
               pnl->error_message);
            Pk_ratio[kin] = pklin[kin]/Pk_nowiggle[kin];
            pnw->lnk_wiggle_tab[kin] = lnki;
   }
   smooth_array_Gaussian(pnw->lnk_wiggle_tab, Pk_ratio, ppr->nk_wiggle, sigma);
    for (kin=0; kin<ppr->nk_wiggle; kin++) {
        pnw->Pk_wiggle_tab[kin] = pklin[kin]-Pk_ratio[kin]*Pk_nowiggle[kin];
    }
    free(Pk_nowiggle);
    free(Pk_ratio);
    free(pklin);
...
double _f_Mead(double x, double y, double p10, double p11, double p12, double p13) {
    return p10 + p11 * (1-x) + p12 * (1-x) * (1-x) + p13 * (1-y);
}
...
double dc_Mead(double a, double Om_m, double f_nu, double g, double G) {
    ...
    double dc_Mead = dc0 * (1. + pow(log10(Om_m), a1) * _f_Mead(g/a, G/a, p10, p11, p12, p13) + _f_Mead(g/a, G/a, p20, p21, p22, p23)) * (1. - 0.041 * f_nu);
    return dc_Mead;
}
...
double Dv_Mead(double a, double Om_m, double f_nu, double g, double G) {
    ...
    */
    ...
    double a3 = 1, a4 = 2;
    double Dv0 = 18.0 * M_PI*M_PI;

    // Halo virial overdensity
    double Dv = 1.0;
    Dv = Dv + _f_Mead(g / a, G / a, p30, p31, p32, p33) * pow(log10(Om_m), a3);
    Dv = Dv + _f_Mead(g / a, G / a, p40, p41, p42, p43) * pow(log10(Om_m), a4);
...
int nonlinear_hmcode2020_auxiliary(
                     struct precision *ppr,
                     struct background *pba,
                     struct perturbs *ppt,
                     struct primordial *ppm,
      ...
      /* Background parameters */
      double Omega_m,fnu,Omega0_m, Omega0_b,Omega0_c, Omega_b, Omega_c, rho_bar;
      double z_at_tau;
      double rho_crit_today_in_msun_mpc3;
      double growth;
      double intgrowth;
      ...
      /* temporary numbers */
      double m, r, nu, sig, sigf, fg, fstar, Mb;
      double diff, r1, r2;
      ...
      double sigma_disp, sigma_disp100, sigma8, sigma8cc, gnorm;
      double delta_c, Delta_v;
      double fraction;
      ...
      Omega_m = pvecback[pba->index_bg_Omega_m];//TBC (i.e. check if for P_cb here we should use Omega_cb) here the total time varying Omega_m is used for delta_c and for Delta_v according to the Mead fit of the Massara simulations.
      Omega_c = pvecback[pba->index_bg_rho_cdm]/pvecback[pba->index_bg_rho_crit];
      Omega_b = pvecback[pba->index_bg_rho_b]/pvecback[pba->index_bg_rho_crit]; 
      rho_bar = pvecback[pba->index_bg_rho_crit]*Omega_m;

      growth = pvecback[pba->index_bg_D];

      z_at_tau = 1./pvecback[pba->index_bg_a]-1.;
      class_call(array_interpolate_two_arrays_one_column(pnw->meadscaletable,
                                                           pnw->intgrowtable,
                                                           1,
                                                           0,
                                                           ppr->n_hmcode_mead_growth_tables,
      ...
      if (imead==5){
        ...
        fstar = (0.01*(2.01-0.30*theta_tagn))*pow(10, z_at_tau*(0.409+0.0224*theta_tagn));
        if(fstar>Omega0_b/Omega0_m) fstar = Omega0_b/Omega0_m;
        Mb = pow(10, 13.87+1.81*theta_tagn+z_at_tau*(-0.108+0.195*theta_tagn))/pba->h; // unit Msun
      }
      else if (imead==4){
        pnl->c_min =4;
      }
      ...
      //delta_c = delta_c*(1.+0.0123*log10(Omega_m)); //Nakamura & Suto (1997) fitting formula for LCDM models (as in Mead 2016)
      //delta_c = delta_c*(1.+0.262*fnu); //Mead et al. (2016; arXiv 1602.02154) neutrino addition
      delta_c = dc_Mead(pvecback[pba->index_bg_a], Omega_m, fnu, meadgrowth, intgrowth);//1.66737;//dc_Mead(pvecback[pba->index_bg_a], Omega_m, fnu, meadgrowth, intgrowth); // Mead et al. (2017; arXiv 1606.05345)
      //printf("%le %le %le %le %le %le \n", pvecback[pba->index_bg_a], Omega_m,  delta_c, fnu, meadgrowth,intgrowth);

      // virialized overdensity
      //Delta_v=418.*pow(Omega_m, -0.352); //Mead et al. (2015; arXiv 1505.07833)
      //Delta_v=Delta_v*(1.+0.916*fnu); //Mead et al. (2016; arXiv 1602.02154) neutrino addition
      Delta_v= Dv_Mead(pvecback[pba->index_bg_a], Omega_m, fnu, meadgrowth, intgrowth);//Dv_Mead(pvecback[pba->index_bg_a], Omega_m, fnu, meadgrowth, intgrowth); 
      //printf("%le %le %le %le %le %le \n", pvecback[pba->index_bg_a], Omega_m,  Delta_v, fnu, meadgrowth,intgrowth);
      ...
      *k_nl = 1./r_nl;
      ...
      k_star = 0.05618*pow(sigma8cc, -1.013)*pba->h; //table 2 of 2009.01858 (Mead) in Mpc
      nd = 2.853;
      kd = 0.05699*pow(sigma8cc, -1.089)*pba->h; //Mpc
    ...
    // growth factor
      ...
      for (index_k = 0; index_k < pnl->k_size; index_k++){
        ...
        pk_lin = exp(lnpk_l[index_pk][index_k])*pow(pnl->k[index_k],3)*anorm; //convert P_k to Delta_k^2
        if (log(pnl->k[index_k])>ppr->logkmax_wiggle+log(pba->h)) pk_wiggle=0;
        else if (log(pnl->k[index_k])<ppr->logkmin_wiggle+log(pba->h)) pk_wiggle=0;
        else{
        ...
        for (index_mass=0; index_mass<index_cut; index_mass++){ //Calculates the integrand for the ph1 integral at all nu values
          ...
          //  if ((pnl->k[index_k]>9)&&(pvecback[pba->index_bg_a]>0.99))
          //  printf("Tagn %le, %le %le %le %le %le %le  %le \n\n\n " , pnl->k[index_k], log10(mass[index_mass]), pnl->hmcode2020log10tagn, Omega_b, Omega_m, Omega_c,  window_nfw*(Omega_c/Omega_m+fg), fstar);
          } 
          else{
            window_nfw*= (1-fnu);
          }
        ...
        fdamp =  1-exp(-1*pnl->k[index_k]*pnl->k[index_k]*sigma_disp*sigma_disp);
        if (fdamp<1.e-3) fdamp=1.e-3;
        if (fdamp>0.99)  fdamp=0.99;
        if((imead==4)||(imead==5)){
            pk_2h=pk_lin;
        ...
        result[index_k] =pow((pow(pk_1h, alpha) + pow(pk_2h, alpha)), (1./alpha))/pow(pnl->k[index_k],3)/anorm;

        free(p1h_integrand);
      }

      // print parameter values
      if ((pnl->nonlinear_verbose > 1 ) || pnl->nonlinear_verbose > 3){
        fprintf(stdout, " -> Parameters at redshift z = %e:\n", z_at_tau);
        fprintf(stdout, "    fnu:		%e\n", fnu);
        fprintf(stdout, "    sigd [Mpc/h]:	%e\n", sigma_disp*pba->h);
        fprintf(stdout, "    sigd100 [Mpc/h]:    %e\n", sigma_disp100*pba->h);
        fprintf(stdout, "    sigma8:		%e\n", sigma8);
        fprintf(stdout, "    sigma8 cc:		%e\n", sigma8cc);
        fprintf(stdout, "    nu min:		%e\n", nu_arr[0]);
        fprintf(stdout, "    nu max:		%e\n", nu_arr[ppr->nsteps_for_p1h_integral-1]);
        fprintf(stdout, "    r_v min [Mpc/h]:    %e\n", r_virial[0]*pba->h);
        fprintf(stdout, "    r_v max [Mpc/h]:    %e\n", r_virial[ppr->nsteps_for_p1h_integral-1]*pba->h);
        ...
        fprintf(stdout, "    k*:			%e\n", k_star/pba->h);
        fprintf(stdout, "    Abary:		%e\n", pnl->c_min);
        fprintf(stdout, "    sigma_disp:		%e\n", sigma_disp*pba->h);
        fprintf(stdout, "    alpha:		%e\n", alpha);
        fprintf(stdout, "    kd (h/Mpc):            %e\n", kd/pba->h);
        fprintf(stdout, "    2halo f:            %e\n", f);
        fprintf(stdout, "    ksize, kmin, kmax:   %d, %e, %e\n", pnl->k_size, pnl->k[0]/pba->h, pnl->k[pnl->k_size-1]/pba->h);
    }
      free(conc);
      free(mass);
...
int nonlinear_hmcode2020(
      ...
      double * result;
      int index_k;
      class_alloc(result, sizeof(double)*pnl->k_size,
                pnl->error_message);
      if (pnl->feedback==hmcode2020tagn){
         double * result1, *result2;
         class_alloc(result1, sizeof(double)*pnl->k_size,
                pnl->error_message);
         class_alloc(result2, sizeof(double)*pnl->k_size,
                pnl->error_message);
        nonlinear_hmcode2020_auxiliary(ppr, pba, ppt, ppm, pnl, index_pk, index_tau, tau, pk_nl, lnpk_l,ddlnpk_l,k_nl,nl_corr_not_computable_at_this_k,pnw,3, result); 
        nonlinear_hmcode2020_auxiliary(ppr, pba, ppt, ppm, pnl, index_pk, index_tau, tau, pk_nl, lnpk_l,ddlnpk_l,k_nl,nl_corr_not_computable_at_this_k,pnw,4, result1); 
        nonlinear_hmcode2020_auxiliary(ppr, pba, ppt, ppm, pnl, index_pk, index_tau, tau, pk_nl, lnpk_l,ddlnpk_l,k_nl,nl_corr_not_computable_at_this_k,pnw,5, result2); 

        for (index_k = 0; index_k < pnl->k_size; index_k++){ 
            pk_nl[index_k] = result[index_k]*result2[index_k]/result1[index_k]; 
        }
        free(result1);
        free(result2);
      }
