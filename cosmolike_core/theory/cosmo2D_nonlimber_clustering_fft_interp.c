...
double w_tomo_fullsky(int nt, int ni, int nj){
    ...
    if (Pl ==0){
        ...
        for(i=0; i<like.Ntheta ; i++){
            ...
            xmin[i]=cos(like.vtmin+i*dt);
          	xmax[i]=cos(like.vtmin+(i+1.0)*dt);
        }
        for (i = 0; i<NTHETA; i ++){
...
//                  if (l < 20){Cl[l]=C_cl_RSD_nointerp(l,nz,nz);}
                    if (l < 20){Cl[l]=C_cl_tomo_nonlimber_interp(l,nz,nz);}
                    else Cl[l]=C_cl_tomo_nonlimber_interp(1.0*l,nz,nz);
                }
                for (i = 0; i < NTHETA; i++){
